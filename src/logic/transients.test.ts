...
describe('Transients Module', () => {
  describe('pressureSurge', () => {
    ...
    it('should throw error for invalid inputs', () => {
      expect(() => pressureSurge(0, 1200, 2)).toThrow('Fluid density must be positive');
      expect(() => pressureSurge(998, 0, 2)).toThrow('Wave speed must be positive');
      expect(() => pressureSurge(-998, 1200, 2)).toThrow('Fluid density must be positive');
    });
  });
  ...
  describe('calculateWaveSpeed', () => {
    ...
    it('should throw error for invalid material properties', () => {
      expect(() => calculateWaveSpeed(0, 2.2e9, 200e9, 0.2, 0.01)).toThrow('Fluid density must be positive');
      expect(() => calculateWaveSpeed(998, 0, 200e9, 0.2, 0.01)).toThrow('Fluid bulk modulus must be positive');
      expect(() => calculateWaveSpeed(998, 2.2e9, 0, 0.2, 0.01)).toThrow('Pipe elastic modulus must be positive');
      expect(() => calculateWaveSpeed(998, 2.2e9, 200e9, 0, 0.01)).toThrow('Pipe diameter must be positive');
      expect(() => calculateWaveSpeed(998, 2.2e9, 200e9, 0.2, 0)).toThrow('Wall thickness must be positive');
    });
  });
  ...
  describe('joukowskySurge - Steel vs Plastic Lines', () => {
    ...
    it('should compare steel vs plastic surge for same conditions', () => {
      ...
      expect(steelResult.waveSpeed.value).toBeGreaterThan(pvcResult.waveSpeed.value);
      expect(steelResult.pressureSurge.value).toBeGreaterThan(pvcResult.pressureSurge.value);
      
      // Steel wave speed should be roughly 2-5x higher than PVC
      expect(steelResult.waveSpeed.value / pvcResult.waveSpeed.value).toBeGreaterThan(2);
      expect(steelResult.waveSpeed.value / pvcResult.waveSpeed.value).toBeLessThan(6);
    });
    ...
    it('should use provided wave speed when available', () => {
      ...
      expect(result.warnings.some(w => 
        typeof w === 'string' && w.includes('Using provided wave speed')
      )).toBe(true);
    });
    ...
    it('should throw error when neither wave speed nor pipe properties are provided', () => {
      ...
      expect(() => joukowskySurge(input)).toThrow('Either wave speed or complete pipe properties');
    });
  });
  ...
  describe('Edge Cases and Validation', () => {
    ...
    it('should handle different fluids', () => {
      ...
      expect(oilResult.pressureSurge.value).toBeLessThan(waterResult.pressureSurge.value);
      expect(oilResult.pressureSurge.value / waterResult.pressureSurge.value).toBeCloseTo(850/998, 2);
    });
