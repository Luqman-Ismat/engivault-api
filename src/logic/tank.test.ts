...
describe('Tank Module', () => {
  describe('calculateOrificeFlow', () => {
    ...
    it('should throw error for invalid coefficient', () => {
      expect(() => calculateOrificeFlow(1.5, 0.01, 2.0)).toThrow('Discharge coefficient must be between 0 and 1');
      expect(() => calculateOrificeFlow(-0.1, 0.01, 2.0)).toThrow('Discharge coefficient must be between 0 and 1');
    });

    it('should throw error for invalid area', () => {
      expect(() => calculateOrificeFlow(0.6, -0.01, 2.0)).toThrow('Orifice area must be positive');
      expect(() => calculateOrificeFlow(0.6, 0, 2.0)).toThrow('Orifice area must be positive');
    });
  });
  ...
  describe('calculateOutflow', () => {
    ...
    it('should throw error for missing orifice configuration', () => {
      ...
      expect(() => calculateOutflow(outflow, 2.0)).toThrow('Orifice configuration required for orifice outflow');
    });

    it('should throw error for missing pump configuration', () => {
      const outflow: OutflowCurve = { type: 'pump' };
      expect(() => calculateOutflow(outflow, 2.0)).toThrow('Pump configuration required for pump outflow');
    });

    it('should throw error for missing constant configuration', () => {
      const outflow: OutflowCurve = { type: 'constant' };
      expect(() => calculateOutflow(outflow, 2.0)).toThrow('Constant flow configuration required for constant outflow');
    });
  });
  ...
  describe('determinePumpState', () => {
    ...
    it('should maintain pump state for non-pump outflow', () => {
      const outflow: OutflowCurve = { type: 'orifice', orifice: { coefficient: 0.6, area: { value: 0.01, unit: 'm²' } } };
  ...
  describe('simulateTank - Constant Inflow and Orifice Outflow', () => {
    ...
    it('should handle tank overflow scenario', () => {
      ...
      expect(result.warnings.some(w => typeof w === 'object' && 'message' in w && w.message.includes('overflow'))).toBe(true);
    });
    ...
    it('should handle tank empty scenario', () => {
      ...
      expect(result.warnings.some(w => typeof w === 'object' && 'message' in w && w.message.includes('empty'))).toBe(true);
    });
    ...
    it('should handle pump with on/off control', () => {
      ...
      const stateChanges = pumpStates.filter((state, index) => index > 0 && state !== pumpStates[index - 1]);
      expect(stateChanges.length).toBeGreaterThan(0);
      
      // Verify pump control logic
      const highLevelPoints = result.timeSeries.filter(point => point.level >= 4);
      const lowLevelPoints = result.timeSeries.filter(point => point.level <= 2);
    ...
    it('should validate input parameters', () => {
      ...
      expect(() => simulateTank(invalidInput)).toThrow('Tank area must be positive');
    });
    ...
    it('should validate time step constraints', () => {
      ...
      expect(() => simulateTank(invalidInput)).toThrow('Time step cannot be greater than end time');
    });
    ...
    it('should validate initial level bounds', () => {
      ...
      expect(() => simulateTank(invalidInput)).toThrow('Initial level must be within tank bounds');
    });
  });
});
