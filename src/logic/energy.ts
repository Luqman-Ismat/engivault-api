...
export function calculatePowerWithBreakdown(
  ...
  // Losses breakdown
  const pumpLosses =
    (hydraulicPower * (1 - efficiencies.pumpEfficiency)) /
    efficiencies.pumpEfficiency;
  const motorLosses =
    ((hydraulicPower / efficiencies.pumpEfficiency) *
      (1 - efficiencies.motorEfficiency)) /
    efficiencies.motorEfficiency;
  ...
  if (efficiencies.vfdEfficiency !== undefined) {
    vfdLosses =
      ((hydraulicPower /
        (efficiencies.pumpEfficiency * efficiencies.motorEfficiency)) *
        (1 - efficiencies.vfdEfficiency)) /
      efficiencies.vfdEfficiency;
  }
