...
describe('Relief Valve Sizing Logic', () => {
  describe('liquidReliefArea', () => {
    ...
    it('should throw error for invalid inputs', () => {
      expect(() => liquidReliefArea(0, 50, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => liquidReliefArea(100, -10, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => liquidReliefArea(100, 50, 0, 0.65)).toThrow('All parameters must be positive');
      expect(() => liquidReliefArea(100, 50, 1.0, 0)).toThrow('All parameters must be positive');
    });
  });
  ...
  describe('criticalPressureRatio', () => {
    ...
    it('should throw error for invalid specific heat ratio', () => {
      expect(() => criticalPressureRatio(1.0)).toThrow('Specific heat ratio must be greater than 1');
      expect(() => criticalPressureRatio(0.8)).toThrow('Specific heat ratio must be greater than 1');
    });
  });
  ...
  describe('backPressureCorrection', () => {
    ...
    it('should throw error for invalid back pressure ratio', () => {
      expect(() => backPressureCorrection(-0.1, 1.4)).toThrow('Back pressure ratio must be between 0 and 1');
      expect(() => backPressureCorrection(1.1, 1.4)).toThrow('Back pressure ratio must be between 0 and 1');
    });
  });
  ...
  describe('gasReliefArea', () => {
    it('should calculate choked gas relief area', () => {
      ...
      const result = gasReliefArea(Q, upstreamPressure, backPressure, temperature, molecularWeight, k, Z, Kd);
    ...
    it('should handle different molecular weights', () => {
      ...
      const resultAir = gasReliefArea(Q, upstreamPressure, backPressure, temperature, 29, k, Z, Kd);
      const resultMethane = gasReliefArea(Q, upstreamPressure, backPressure, temperature, 16, k, Z, Kd);
    ...
    it('should handle different temperatures', () => {
      ...
      const resultCold = gasReliefArea(Q, upstreamPressure, backPressure, 460, molecularWeight, k, Z, Kd);
      const resultHot = gasReliefArea(Q, upstreamPressure, backPressure, 600, molecularWeight, k, Z, Kd);
    ...
    it('should throw error for invalid inputs', () => {
      expect(() => gasReliefArea(0, 100, 50, 530, 29, 1.4, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => gasReliefArea(1000, 0, 50, 530, 29, 1.4, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => gasReliefArea(1000, 100, 50, 0, 29, 1.4, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => gasReliefArea(1000, 100, 50, 530, 0, 1.4, 1.0, 0.65)).toThrow('All parameters must be positive');
      expect(() => gasReliefArea(1000, 100, 120, 530, 29, 1.4, 1.0, 0.65)).toThrow('Back pressure must be less than upstream pressure');
    });
  });
  ...
  describe('sizeReliefValve', () => {
    ...
    it('should generate warnings for extreme conditions', () => {
      ...
      expect(result.warnings.some(w => typeof w === 'string' && w.includes('Low pressure drop'))).toBe(true);
      expect(result.warnings.some(w => typeof w === 'string' && w.includes('Specific gravity above 2.0'))).toBe(true);
      expect(result.warnings.some(w => typeof w === 'string' && w.includes('Low discharge coefficient'))).toBe(true);
    });
    ...
    it('should throw error for missing gas parameters', () => {
      ...
      expect(() => sizeReliefValve(input)).toThrow('Molecular weight must be provided and positive for gas flow');
    });
    ...
    it('should throw error for invalid pressure relationship', () => {
      ...
      expect(() => sizeReliefValve(input)).toThrow('Back pressure must be less than upstream pressure');
    });
  ...
  describe('validateReliefValveInputs', () => {
    ...
    it('should reject invalid fluid type', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Fluid type must be either "liquid" or "gas"');
    });
    ...
    it('should reject missing gas parameters', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Molecular weight must be provided and positive for gas flow');
      expect(result.errors).toContain('Temperature must be provided for gas flow');
      expect(result.errors).toContain('Back pressure must be provided for gas flow');
    });
    ...
    it('should reject invalid pressure relationship', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Back pressure must be less than upstream pressure');
    });
    ...
    it('should reject invalid coefficients', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Discharge coefficient must be between 0 and 1');
      expect(result.errors).toContain('Back pressure correction factor must be between 0 and 1');
      expect(result.errors).toContain('Compressibility factor must be positive');
      expect(result.errors).toContain('Specific heat ratio must be greater than 1');
    });
  });
});
