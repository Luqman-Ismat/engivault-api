 ...
 * @returns Required Cv
 */
export function requiredCvLiquid(Q: number, deltaP: number, SG: number): number {
  if (deltaP <= 0) {
 ...
 */
export function criticalPressureDrop(FL: number, P1: number, Pv: number): number {
  if (FL <= 0 || FL > 1) {
 ...
 */
export function checkChokedFlow(actualDeltaP: number, criticalDeltaP: number): {
  isChoked: boolean;
  likelihood: 'none' | 'low' | 'medium' | 'high';
} {
 ...
 */
export function getDefaultPressureRecoveryFactor(trimCharacteristic: ValveTrimCharacteristic): number {
 ...
 */
export function getVaporPressure(fluidName: string, temperature: number): number {
  ...
  switch (fluidName.toLowerCase()) {
    ...
    case 'ethanol':
      ...
      if (T_C_ethanol >= 0 && T_C_ethanol <= 100) {
        const P_vap_mmHg = Math.pow(10, 8.20417 - 1642.89 / (230.3 + T_C_ethanol));
...
export function sizeValve(input: ValveSizingInput): ValveSizingResult {
  ...
  if (input.upstreamPressure) {
    const FL = input.pressureRecoveryFactor || getDefaultPressureRecoveryFactor(input.trimCharacteristic);
    const P1 = convert(input.upstreamPressure, 'Pa').value;
    ...
    if (chokedAnalysis.likelihood === 'high') {
      warnings.push('High likelihood of choked flow - consider larger valve or higher upstream pressure');
    } else if (chokedAnalysis.likelihood === 'medium') {
      warnings.push('Medium likelihood of choked flow - monitor valve performance');
    }
  }
  
  // Generate warnings for valve authority
  if (valveAuthority < 0.2) {
    warnings.push('Low valve authority (< 0.2) - poor control characteristics expected');
  } else if (valveAuthority > 0.8) {
    warnings.push('High valve authority (> 0.8) - valve dominates system pressure drop');
  }
  
  // Generate warnings for Cv size
  if (cv < 1) {
    warnings.push('Very small Cv (< 1) - consider smaller valve or higher pressure drop');
  } else if (cv > 1000) {
    warnings.push('Very large Cv (> 1000) - consider larger valve or lower pressure drop');
  }
  ...
  if (deltaP_psi < 1) {
    ...
    warnings.push('High pressure drop (> 100 psi) - verify valve rating and system design');
  }
 ...
 */
export function validateValveSizingInputs(input: ValveSizingInput): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (input.upstreamPressure && input.downstreamPressure) {
    ...
    if (input.pressureDrop) {
      ...
      if (deltaP > actualDeltaP) {
        errors.push('Specified pressure drop cannot exceed actual pressure drop');
      }
    }
  }
