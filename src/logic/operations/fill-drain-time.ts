...
export function computeFillDrainTime(input: FillDrainInput): FillDrainResult {
  ...
  const timeStep = convert(input.timeStep ?? { value: 60, unit: 's' }, 's').value;
  ...
  if (input.operation === 'fill' && initialLevel >= targetLevel) {
    throw new Error('For fill operation, initial level must be less than target level');
  }
  
  if (input.operation === 'drain' && initialLevel <= targetLevel) {
    throw new Error('For drain operation, initial level must be greater than target level');
  }
  ...
  while (currentTime < maxTime) {
    ...
    if (Math.abs(flowRate) < 1e-10) {
      warnings.push({
        type: 'flow_rate',
        message: 'Flow rate is very small, operation may take a very long time.',
        severity: 'medium'
      });
      break;
    }
    ...
    if (newVolume < 0) {
      warnings.push({
        type: 'tank_capacity',
        message: 'Tank would be completely drained. Operation stopped at empty tank.',
        severity: 'high'
      });
      break;
    }
    ...
    if (input.operation === 'fill' && newLevel >= targetLevel) {
      ...
      totalVolume = targetLevel * crossSectionalArea - initialLevel * crossSectionalArea;
    ...
    if (input.operation === 'drain' && newLevel <= targetLevel) {
      ...
      totalVolume = initialLevel * crossSectionalArea - targetLevel * crossSectionalArea;
      totalFlowTime = currentTime;
      break;
    }
  ...
  if (currentTime >= maxTime) {
    warnings.push({
      type: 'time_limit',
      message: 'Operation did not complete within 24 hours. Consider increasing flow rate or checking parameters.',
      severity: 'high'
    });
  }
  ...
  return {
    ...
    metadata: {
      input,
      calculations: {
        method: input.flowRate.type === 'constant' ? 'Constant flow rate' : 'Variable flow rate',
