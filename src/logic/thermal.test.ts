...
describe('Thermal Module', () => {
  describe('waterProps', () => {
    ...
    it('should throw error for temperature below 0°C', () => {
      expect(() => waterProps(270)).toThrow('Temperature must be between 273.15K (0°C) and 373.15K (100°C)');
    });

    it('should throw error for temperature above 100°C', () => {
      expect(() => waterProps(380)).toThrow('Temperature must be between 273.15K (0°C) and 373.15K (100°C)');
    });
  });
  ...
  describe('temperatureDependentViscosity', () => {
    it('should calculate viscosity at different temperatures', () => {
      ...
      const viscosity20 = temperatureDependentViscosity(baseViscosity, referenceTemperature, 293.15);
      const viscosity40 = temperatureDependentViscosity(baseViscosity, referenceTemperature, 313.15);
      const viscosity60 = temperatureDependentViscosity(baseViscosity, referenceTemperature, 333.15);
    ...
    it('should handle temperature decrease', () => {
      ...
      const viscosity10 = temperatureDependentViscosity(baseViscosity, referenceTemperature, 283.15);
      const viscosity0 = temperatureDependentViscosity(baseViscosity, referenceTemperature, 273.15);
    ...
    it('should throw error for invalid inputs', () => {
      expect(() => temperatureDependentViscosity(0, 293.15, 313.15)).toThrow('All parameters must be positive');
      expect(() => temperatureDependentViscosity(1e-3, 0, 313.15)).toThrow('All parameters must be positive');
      expect(() => temperatureDependentViscosity(1e-3, 293.15, 0)).toThrow('All parameters must be positive');
    });
  });
  ...
  describe('calculateAverageTemperature', () => {
    ...
    it('should throw error for invalid temperatures', () => {
      expect(() => calculateAverageTemperature(0, 298.15)).toThrow('Temperatures must be positive');
      expect(() => calculateAverageTemperature(298.15, 0)).toThrow('Temperatures must be positive');
    });
  });
  ...
  describe('estimateOutletTemperature', () => {
    it('should estimate outlet temperature for typical conditions', () => {
      ...
      const outletTemp = estimateOutletTemperature(inletTemp, flowRate, diameter, length);
    ...
    it('should handle high flow rates', () => {
      ...
      expect(outletTemp).toBeGreaterThan(estimateOutletTemperature(inletTemp, 0.01, diameter, length));
    });
    ...
    it('should handle long pipes', () => {
      ...
      expect(outletTemp).toBeLessThan(estimateOutletTemperature(inletTemp, flowRate, diameter, 100));
    });

    it('should throw error for invalid inputs', () => {
      expect(() => estimateOutletTemperature(0, 0.01, 0.05, 100)).toThrow('All parameters must be positive');
      expect(() => estimateOutletTemperature(353.15, 0, 0.05, 100)).toThrow('All parameters must be positive');
      expect(() => estimateOutletTemperature(353.15, 0.01, 0, 100)).toThrow('All parameters must be positive');
      expect(() => estimateOutletTemperature(353.15, 0.01, 0.05, 0)).toThrow('All parameters must be positive');
    });
  });
  ...
  describe('iterateViscosityAdjustedDrop', () => {
    ...
    it('should show convergence behavior', () => {
      ...
      const pressureDrops = result.metadata.calculations.convergenceHistory.map(h => h.pressureDrop);
      ...
      if (pressureDrops.length > 1) {
        const change = Math.abs(lastDrop - secondLastDrop) / Math.max(lastDrop, 1e-6);
    ...
    it('should handle high temperature differences', () => {
      ...
      expect(result.converged).toBe(true);
      expect(result.averageViscosity.value).toBeLessThan(baseFluid.viscosity.value); // Average should be less than reference
    });
    ...
    it('should throw error for invalid inputs', () => {
      ...
      expect(() => iterateViscosityAdjustedDrop(input)).toThrow('Flow rate, length, and diameter must be positive');
    });
