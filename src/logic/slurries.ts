...
export function archimedesNumber(
  ...
  return (densityDifference * fluidDensity * G * Math.pow(particleDiameter, 3)) / Math.pow(fluidViscosity, 2);
}
...
export function settlingVelocity(
...
): { velocity: number; method: 'stokes' | 'intermediate' | 'newton' | 'durand'; dragCoeff: number } {
  const warnings: (string | Warning)[] = [];
  
  // Validate inputs
  if (particleDensity <= 0 || fluidDensity <= 0 || particleDiameter <= 0 || fluidViscosity <= 0) {
    throw new Error('All input parameters must be positive');
  }
  
  if (particleDensity <= fluidDensity) {
    throw new Error('Particle density must be greater than fluid density for settling');
  }
  ...
  if (ar < 36) {
    // Stokes regime
    velocity = (particleDensity - fluidDensity) * G * Math.pow(particleDiameter, 2) / (18 * fluidViscosity);
    method = 'stokes';
    dragCoeff = 24; // For Re < 0.1
  } else if (ar < 83000) {
    // Intermediate regime
    velocity = Math.pow(4 * (particleDensity - fluidDensity) * G * particleDiameter / (3 * fluidDensity), 0.5) * 
               Math.pow(ar / (18 + 0.6 * Math.pow(ar, 0.5)), 0.5);
    method = 'intermediate';
    dragCoeff = 24 * (1 + 0.15 * Math.pow(ar / 24, 0.687));
  } else {
    // Newton regime
    velocity = Math.pow(3.1 * (particleDensity - fluidDensity) * G * particleDiameter / fluidDensity, 0.5);
    method = 'newton';
    dragCoeff = 0.44;
  }
  ...
  return { velocity, method, dragCoeff };
}
...
export function durandFactor(
  ...
  const durandFactor = 180 * (settlingVelocity / flowVelocity) * 
...
export function slurryHeadloss(
  ...
  settlingVelocity: number,
  concentration: number
): { headloss: number; frictionFactor: number; durandFactor: number; froudeNumber: number } {
  const warnings: (string | Warning)[] = [];
  
  // Validate inputs
  if (carrierDensity <= 0 || carrierViscosity <= 0 || flowVelocity <= 0 || 
      pipeDiameter <= 0 || pipeLength <= 0 || pipeRoughness < 0) {
    throw new Error('All input parameters must be positive');
  }
  ...
  const reynoldsNumber = reynolds(carrierDensity, flowVelocity, pipeDiameter, carrierViscosity);
  ...
  const carrierHeadloss = (carrierFrictionFactor * pipeLength * Math.pow(flowVelocity, 2)) / 
                          (2 * G * pipeDiameter);
  ...
  if (concentration > 0.15) {
    warnings.push({
      type: 'warning',
      message: 'High concentration (> 15%): Durand correlation may not be applicable',
      severity: 'medium'
    });
  }
  ...
  // Calculate slurry headloss using Durand correlation
  // i_s = i_w * (1 + C_D * C_v)
  const slurryHeadlossValue = carrierHeadloss * (1 + durandFactorValue * concentration);
 ...
 */
export function calculateSettlingVelocity(input: SlurryFlowInput): SettlingVelocityResult {
  const warnings: (string | Warning)[] = [];
  
  // Convert input parameters to SI units
  const particleDensity = convert(input.slurry.particles.density, 'kg/m³').value;
  const particleDiameter = convert(input.slurry.particles.diameter, 'm').value;
  const fluidDensity = convert(input.slurry.carrierFluid.density, 'kg/m³').value;
  const fluidViscosity = convert(input.slurry.carrierFluid.viscosity, 'Pa·s').value;
  
  // Get shape factor
  const shapeFactor = input.slurry.particles.shape ? SHAPE_FACTORS[input.slurry.particles.shape] : 1.0;
  ...
  // Calculate particle Reynolds number
  const particleRe = particleReynolds(fluidDensity, velocity, particleDiameter, fluidViscosity);
  ...
  if (ar > 100000) {
    warnings.push({
      type: 'warning',
      message: 'Very high Archimedes number: Newton regime with high uncertainty',
      severity: 'medium'
    });
  }
 ...
 */
export function calculateSlurryHeadloss(input: SlurryFlowInput): SlurryHeadlossResult {
  const warnings: (string | Warning)[] = [];
  
  // Convert input parameters to SI units
  const carrierDensity = convert(input.slurry.carrierFluid.density, 'kg/m³').value;
  const carrierViscosity = convert(input.slurry.carrierFluid.viscosity, 'Pa·s').value;
  ...
  if (input.slurry.concentrationType === 'volume') {
    ...
    const particleDensity = convert(input.slurry.particles.density, 'kg/m³').value;
    const weightFraction = convert(input.slurry.concentration, 'dimensionless').value;
    concentration = (weightFraction / particleDensity) / 
                   (weightFraction / particleDensity + (1 - weightFraction) / carrierDensity);
  }
  ...
  const { headloss, frictionFactor, durandFactor: durandFactorValue, froudeNumber } = slurryHeadloss(
    carrierDensity,
    carrierViscosity,
    flowVelocity,
    pipeDiameter,
  ...
  if (particleDiameter / pipeDiameter > 0.1) {
    warnings.push({
      type: 'warning',
      message: 'Large particle-to-pipe diameter ratio: Durand correlation may not be applicable',
      severity: 'high'
    });
  }
