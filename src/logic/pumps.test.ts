...
describe('Pump Module', () => {
  ...
  describe('interpolateCurve', () => {
    ...
    it('should throw error for insufficient points', () => {
      expect(() => interpolateCurve([{ q: 0, h: 100 }])).toThrow(
    ...
    it('should throw error for duplicate flow rates', () => {
      ...
      expect(() => interpolateCurve(curve)).toThrow(
        'Curve contains duplicate flow rates'
      );
    });
  });
  ...
  describe('scaleByAffinity', () => {
    ...
    it('should throw error for negative speed ratio', () => {
      ...
      expect(() => scaleByAffinity(curve, -1)).toThrow(
        'Speed ratio must be positive'
      );
    });
  });
  ...
  describe('bepc', () => {
    ...
    it('should throw error for empty curve', () => {
      ...
      expect(() => bepc(0.1, 80, emptyCurve)).toThrow(
  ...
  describe('bepDistance', () => {
    it('should calculate BEP distance with warnings for far operation', () => {
      ...
      const bepWarnings = result.warnings.filter(
        w => w.type === 'bep_distance'
      );
  ...
  describe('calculatePumpPower', () => {
    ...
    it('should throw error for invalid efficiency', () => {
      expect(() => calculatePumpPower(0.1, 80, 1.5, 1000)).toThrow(
        'Efficiency must be between 0 and 1'
      );
      expect(() => calculatePumpPower(0.1, 80, -0.1, 1000)).toThrow(
        'Efficiency must be between 0 and 1'
      );
    });

    it('should throw error for negative density', () => {
      expect(() => calculatePumpPower(0.1, 80, 0.8, -1000)).toThrow(
  ...
  describe('findOperatingPoint', () => {
    it('should find operating point at intersection', () => {
      ...
      expect(
        Math.abs(pumpCurve(result.q) - systemCurve(result.q))
      ).toBeLessThan(1e-5);
    });
    ...
    it('should throw error for no intersection', () => {
      ...
      expect(() => findOperatingPoint(pumpCurve, systemCurve, 0, 1)).toThrow(
    ...
    it('should throw error for invalid range', () => {
      ...
      expect(() => findOperatingPoint(pumpCurve, systemCurve, 1, 0)).toThrow(
  ...
  describe('validatePumpCurve', () => {
    ...
    it('should detect insufficient points', () => {
      ...
      const result = validatePumpCurve(curve);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.includes('at least one point'))).toBe(
        true
      );
    });
  });
