 ...
 */
export function hxPressureDrop(
  input: HXPressureDropInput
  ...
  if (
    tubeDiameter <= 0 ||
    tubeLength <= 0 ||
    input.geometry.numberOfTubes <= 0
  ) {
    throw new Error(
...
function calculateTubeSidePressureDrop(
  ...
  const totalLength =
    tubeLength * input.passes + returnBendEquivalentLength * (input.passes - 1);

  // Calculate pressure drop using Darcy-Weisbach equation
  const pressureDrop =
    (frictionFactor * totalLength * density * Math.pow(velocity, 2)) /
    (2 * tubeDiameter);
  ...
  if (reynoldsNumber < 1000) {
    warnings.push({
      type: 'general',
      message:
        'Very low Reynolds number, laminar flow correlations may not be accurate',
      severity: 'medium',
    });
  }
...
function calculateShellSidePressureDrop(
  ...
  const tubeArea =
    ((Math.PI * Math.pow(tubeOuterDiameter, 2)) / 4) *
    input.geometry.numberOfTubes;
  ...
  if (input.geometry.tubeLayout === 'triangular') {
    equivalentDiameter =
      (4 *
        ((Math.pow(tubePitch, 2) * Math.sqrt(3)) / 4 -
          (Math.PI * Math.pow(tubeOuterDiameter, 2)) / 8)) /
  ...
  // 1. Cross-flow pressure drop
  const crossFlowPressureDrop =
    (frictionFactor * numberOfCrossings * density * Math.pow(velocity, 2)) / 2;

  // 2. Window-flow pressure drop (simplified)
  const windowPressureDrop =
    0.5 * density * Math.pow(velocity, 2) * numberOfCrossings;
  ...
  // Total pressure drop
  const pressureDrop =
    crossFlowPressureDrop + windowPressureDrop + entranceExitPressureDrop;
