 ...
 */
export function liquidReliefArea(Q: number, deltaP: number, SG: number, Kd: number = 0.65): number {
 ...
 */
export function backPressureCorrection(backPressureRatio: number, k: number = 1.4): number {
...
export function gasReliefArea(
  ...
  Z: number = 1.0,
  Kd: number = 0.65
): { area: number; regime: 'choked-gas' | 'non-choked-gas'; criticalRatio: number; Kb: number } {
  if (Q <= 0 || upstreamPressure <= 0 || temperature <= 0 || molecularWeight <= 0) {
    throw new Error('All parameters must be positive');
  }
  ...
  if (backPressureRatio <= criticalRatio) {
    ...
    area = (Q * Math.sqrt(temperature * molecularWeight / (upstreamPressure * Kd * Kb * C)));
  } else {
    // Non-choked flow
    regime = 'non-choked-gas';
    // Simplified non-choked equation
    const pressureTerm = Math.sqrt(1 - Math.pow(backPressureRatio, (k - 1) / k));
    area = (Q * Math.sqrt(temperature * molecularWeight / upstreamPressure)) / (Kd * Kb * 520 * pressureTerm);
  }
...
export function sizeReliefValve(input: ReliefValveInput): ReliefValveResult {
  ...
  const Q = convert(input.flow, input.fluidType === 'liquid' ? 'gpm' : 'scfm').value;
  ...
  if (input.fluidType === 'liquid') {
    ...
    if (!input.molecularWeight || input.molecularWeight <= 0) {
      throw new Error('Molecular weight must be provided and positive for gas flow');
    }
    ...
    if (temperature < 400 || temperature > 2000) {
      warnings.push('Temperature outside typical range (400-2000 R) - verify conditions');
    }
    
    if (molecularWeight < 2 || molecularWeight > 200) {
      warnings.push('Molecular weight outside typical range (2-200) - verify fluid');
    }
    
    if (k < 1.1 || k > 1.7) {
      warnings.push('Specific heat ratio outside typical range (1.1-1.7) - verify fluid');
    }
    
    if (Z < 0.5 || Z > 1.5) {
      warnings.push('Compressibility factor outside typical range (0.5-1.5) - verify conditions');
    }
    
    const gasResult = gasReliefArea(Q, upstreamPressure, backPressure, temperature, molecularWeight, k, Z, Kd);
  ...
  if (requiredArea < 0.1) {
    warnings.push('Very small required area (< 0.1 in²) - consider minimum valve size');
  } else if (requiredArea > 100) {
    warnings.push('Very large required area (> 100 in²) - consider multiple valves or verify conditions');
  }
  ...
  if (deltaP < 1) {
    ...
    warnings.push('High pressure drop (> 1000 psi) - verify valve rating and system design');
  }
  
  // Generate warnings for discharge coefficient
  if (Kd < 0.5) {
    warnings.push('Low discharge coefficient (< 0.5) - verify valve type and conditions');
  }
 ...
 */
export function validateReliefValveInputs(input: ReliefValveInput): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (input.fluidType === 'liquid') {
    ...
    if (!input.molecularWeight || input.molecularWeight <= 0) {
      errors.push('Molecular weight must be provided and positive for gas flow');
    }
