...
function calculatePressureDrop(
  ...
  density: number,
  viscosity: number
): { pressureDrop: number; velocity: number; reynoldsNumber: number; frictionFactor: number } {
...
function sizeForVelocity(
  ...
  density: number,
  viscosity: number
): { diameter: number; pressureDrop: number; velocity: number; reynoldsNumber: number; frictionFactor: number; iterations: number } {
  ...
  while (iterations < MAX_ITERATIONS && tolerance > CONVERGENCE_TOLERANCE) {
    const result = calculatePressureDrop(flow, diameter, length, roughness, density, viscosity);
    const velocityError = Math.abs(result.velocity - targetVelocity) / targetVelocity;
...
function sizeForPressureDrop(
  ...
  const estimates = [
    ...
    Math.pow(0.02 * length * density * flow * flow / (Math.PI * Math.PI * targetPressureDrop), 0.2),
  ...
  for (const estimate of estimates) {
    ...
    if (!isNaN(result.pressureDrop) && result.pressureDrop > 0) {
      const error = Math.abs(result.pressureDrop - targetPressureDrop) / targetPressureDrop;
      if (error < bestError) {
        bestError = error;
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (isNaN(result.pressureDrop) || result.pressureDrop <= 0) {
      ...
      const pressureDrop = assumedFrictionFactor * (length / diameter) * density * velocity * velocity / 2;
      return {
        diameter,
        pressureDrop,
        velocity,
    ...
    const pressureError = Math.abs(result.pressureDrop - targetPressureDrop) / targetPressureDrop;
    
    if (pressureError < 0.01) { // 1% tolerance
    ...
    const adjustmentFactor = Math.pow(targetPressureDrop / result.pressureDrop, 0.05);
    diameter *= adjustmentFactor;
...
export function sizePipeForTarget(input: PipeSizingInput): PipeSizingResult {
  const warnings: Array<{ type: 'transition' | 'mach' | 'correlation' | 'general'; message: string }> = [];
  ...
  const targetValue = convert(input.value, input.target === 'velocity' ? 'm/s' : 'Pa').value;
  const roughness = convert(
    input.roughness ?? DEFAULT_ROUGHNESS,
    'm'
  ).value;
  ...
  let result: { diameter: number; pressureDrop: number; velocity: number; reynoldsNumber: number; frictionFactor: number; iterations: number };
  let finalTolerance = 0;
  
  if (input.target === 'velocity') {
    result = sizeForVelocity(flow, targetValue, length, roughness, density, viscosity);
    finalTolerance = Math.abs(result.velocity - targetValue) / targetValue;
  } else {
    result = sizeForPressureDrop(flow, targetValue, length, roughness, density, viscosity);
  ...
  if (result.reynoldsNumber < 2300) {
    warnings.push({
      type: 'transition',
      message: 'Flow is laminar (Re < 2300). Consider using different correlations for laminar flow.'
    });
  ...
  if (result.velocity > 3) {
    warnings.push({
      type: 'general',
      message: 'High velocity detected (> 3 m/s). Consider erosion and noise implications.'
    });
  }
  ...
  if (result.diameter < 0.01) {
    warnings.push({
      type: 'general',
      message: 'Very small diameter calculated (< 10 mm). Verify practical feasibility.'
    });
  } else if (result.diameter > 1) {
    warnings.push({
      type: 'general',
      message: 'Large diameter calculated (> 1 m). Verify practical feasibility.'
    });
  }
  ...
  return {
    ...
    metadata: {
      input,
      calculations: {
        method: input.target === 'velocity' ? 'Velocity-based sizing' : 'Pressure drop-based sizing',
