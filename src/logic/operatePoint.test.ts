...
describe('Operating Point Logic', () => {
  ...
  describe('combinePumps', () => {
    ...
    it('should throw error for empty pump array', () => {
      expect(() => combinePumps([], 'single')).toThrow('At least one pump curve is required');
    });

    it('should throw error for invalid speed', () => {
      expect(() => combinePumps([samplePump1], 'single', 0)).toThrow('Speed must be positive');
      expect(() => combinePumps([samplePump1], 'single', -1)).toThrow('Speed must be positive');
    });

    it('should throw error for invalid arrangement', () => {
      expect(() => combinePumps([samplePump1], 'invalid' as any)).toThrow('Invalid arrangement');
    });
  });
  ...
  describe('solveIntersection', () => {
    ...
    it('should throw error for invalid range', () => {
      ...
      expect(() => solveIntersection(pumpCurve, systemCurve, 1, 0)).toThrow('qMin must be less than qMax');
    });
  });

  describe('calculateOperatingPoint', () => {
    it('should calculate operating point for single pump', () => {
      const result = calculateOperatingPoint([samplePump1], 'single', sampleSystem);
      ...
      expect(Math.abs(result.pumpHead.value - result.systemHead.value)).toBeLessThan(1e-4);
    });

    it('should calculate operating point for pumps in series', () => {
      const result = calculateOperatingPoint([samplePump1, samplePump2], 'series', sampleSystem);
      ...
      const singleResult = calculateOperatingPoint([samplePump1], 'single', sampleSystem);
    ...
    it('should calculate operating point for pumps in parallel', () => {
      const result = calculateOperatingPoint([samplePump1, samplePump1], 'parallel', sampleSystem);
    ...
    it('should apply speed scaling', () => {
      const speed = 1.1;
      const result = calculateOperatingPoint([samplePump1], 'single', sampleSystem, speed);
    ...
    it('should handle polynomial system curve', () => {
      ...
      const result = calculateOperatingPoint([samplePump1], 'single', polynomialSystem);
    ...
    it('should generate warnings for invalid curves', () => {
      ...
      expect(() => calculateOperatingPoint([invalidPump], 'single', sampleSystem)).toThrow('Curve must have at least 2 points');
    });

    it('should throw error for empty pump array', () => {
      expect(() => calculateOperatingPoint([], 'single', sampleSystem)).toThrow('At least one pump is required');
    });
  });

  describe('validateOperatingPointInputs', () => {
    it('should validate correct inputs', () => {
      const result = validateOperatingPointInputs([samplePump1], 'single', sampleSystem);
    ...
    it('should reject pump without ID', () => {
      ...
      const result = validateOperatingPointInputs([invalidPump], 'single', sampleSystem);
    ...
    it('should reject pump with insufficient curve points', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Pump pump1: Curve must have at least 2 points');
    });
    ...
    it('should reject invalid arrangement', () => {
      const result = validateOperatingPointInputs([samplePump1], 'invalid' as any, sampleSystem);
    ...
    it('should reject negative static head', () => {
      ...
      const result = validateOperatingPointInputs([samplePump1], 'single', invalidSystem);
    ...
    it('should reject negative system coefficient', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('System coefficient k cannot be negative');
    });
    ...
    it('should validate polynomial coefficients', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Polynomial coefficient 1 must be a number');
    });
  });
});
