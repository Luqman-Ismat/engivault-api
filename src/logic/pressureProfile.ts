...
export function computeProfile(
  ...
  if (fluid.density && fluid.viscosity) {
    ...
    throw new Error(
      'Either fluid properties (density, viscosity) or fluid name must be provided'
    );
  }
  ...
  for (const segment of segments) {
    ...
    const frictionalLoss = darcyWeisbachDP(
      rho,
      velocity,
      length,
      diameter,
    ...
    if (velocity > 10) {
      segmentWarnings.push(
        `High velocity (${velocity.toFixed(1)} m/s) may cause cavitation or erosion`
      );
    }
    ...
    if (velocity < 0.5) {
      segmentWarnings.push(
        `Low velocity (${velocity.toFixed(2)} m/s) may cause sedimentation`
      );
    }
...
export function validateSegments(segments: PipelineSegment[]): {
  isValid: boolean;
  warnings: string[];
} {
  const warnings: string[] = [];
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (roughness > diameter / 10) {
      warnings.push(
        `Segment ${i + 1}: Roughness is very high relative to diameter`
      );
    }
    ...
    if (segment.kLocal) {
      for (let j = 0; j < segment.kLocal.length; j++) {
        if (segment.kLocal[j] < 0) {
          warnings.push(
            `Segment ${i + 1}, K-factor ${j + 1}: Cannot be negative`
          );
        }
      }
    }
...
export function getPressureProfileSummary(
  result: PressureProfileResult
): string {
