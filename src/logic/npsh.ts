import { NpshInput, NpshrCurvePoint } from '../schemas/validation';

export interface NpshOutputs {
  npsha: number; // meters
}

export interface NpshrOutputs {
  npshr: number; // meters
}

export function calculateNpsh(inputs: NpshInput): NpshOutputs {
  const {
    atmosphericPressure,
    vaporPressure,
    fluidDensity,
...
export function calculateNpshr(
  flowRate: number,
  npshrCurve: NpshrCurvePoint[]
): NpshrOutputs {
  ...
  let p1: NpshrCurvePoint | undefined;
  let p2: NpshrCurvePoint | undefined;

  for (let i = 0; i < sortedCurve.length - 1; i++) {
    if (
      flowRate >= sortedCurve[i]!.flow &&
  ...
  if (!p1 || !p2) {
    throw new Error(
      'Could not find interpolation points for the given flow rate.'
    );
  }
