  ...
  describe('POST /api/v1/operations/fill-drain-time', () => {
    ...
    it('should record time history correctly', async () => {
      ...
      for (let i = 1; i < result.timeHistory.length; i++) {
        expect(result.timeHistory[i].time).toBeGreaterThan(
          result.timeHistory[i - 1].time
        );
        expect(result.timeHistory[i].level).toBeGreaterThanOrEqual(
          result.timeHistory[i - 1].level
        );
        expect(result.timeHistory[i].volume).toBeGreaterThanOrEqual(
          result.timeHistory[i - 1].volume
        );
    ...
    describe('Batch Processing', () => {
      ...
      it('should handle batch request with mixed success/failure', async () => {
        ...
        expect(result.errors[0].error).toBe(
          'Diameter is required for cylindrical tanks'
        );
      });
      ...
      it('should handle batch request with all invalid items', async () => {
        ...
        expect(result.errors[1].error).toBe(
          'Constant flow rate must be positive'
        );
      });
      ...
      it('should handle batch request with complex mixed scenarios', async () => {
        ...
        const batchPayload = {
          items: [
            validCylindricalTank,
            validVariableFlow,
            invalidMissingFields,
            invalidOperation,
            validRectangularTank,
          ],
        };
