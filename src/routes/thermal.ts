...
**Version:** 1.0.0`,
    ...
    async (request, reply) => {
      try {
        ...
        if (
          input.flowRate.value <= 0 ||
          input.length.value <= 0 ||
          input.diameter.value <= 0
        ) {
        ...
        if (
          input.fluid.density.value <= 0 ||
          input.fluid.viscosity.value <= 0 ||
          input.fluid.referenceTemperature.value <= 0
        ) {
          return reply.status(400).send({
            error: 'ValidationError',
